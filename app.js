(() => {
  ...
  const state={profile:LearningStore.profile(),level:'debutant',mission:null,rounds:[],index:0,score:0,hearts:5,xpEarned:0,validated:false,selected:null,correct:null,pairFirst:null,pairMistakes:0,sentenceTokens:[],answerTokens:[],quick:false,special:null,recommended:null,installPrompt:null};
  ...
  function write(name,val){LearningStore.write(key(name),val)}
  ...
  function updateStreak(){const s=read('stats',{best:0,played:0,xp:0,streak:0,lastDay:'',dailyDay:''}),today=LearningStore.day();if(s.lastDay!==today){const y=(()=>{const d=new Date();d.setDate(d.getDate()-1);return [d.getFullYear(),String(d.getMonth()+1).padStart(2,'0'),String(d.getDate()).padStart(2,'0')].join('-')})();s.streak=s.lastDay===y?(s.streak||0)+1:1;s.lastDay=today;write('stats',s)}}
  ...
  function renderHome(){if($('resumeQuiz'))$('resumeQuiz').hidden=!LearningStore.getSession('quiz');
  ...
  function bindAudio(text){$('speakNormal').onclick=()=>speak(text,.82);$('speakSlow').onclick=()=>speak(text,.53);speak(text,.82)}
  ...
  function validateRound(){
    ...
    state.validated=true;LearningStore.markActivity(idFor(q),1);updateMastery(q,ok);if(ok){state.score++;state.xpEarned+=state.level==='confirme'?15:state.level==='moyen'?12:10;$('feedback').textContent='Bonne réponse !';$('feedback').className='feedback show good';removeMistake(q);speak('Good job',.85);vibrate(40)}else{state.hearts=Math.max(0,state.hearts-1);$('feedback').textContent='À revoir. Regarde la bonne réponse.';$('feedback').className='feedback show bad';saveMistake(q);speak('Try again',.85);vibrate([80,50,80])}showReveal(q);$('scoreLabel').textContent=`${state.score}/${state.rounds.length}`;$('hearts').textContent='❤'.repeat(state.hearts)+'♡'.repeat(Math.max(0,5-state.hearts));$('validate').textContent='CONTINUER';$('validate').disabled=false;$('progress').style.width=((state.index+1)/state.rounds.length*100)+'%';saveQuiz()
  }
  ...
  function finishSession(){LearningStore.clearSession('quiz');
    const total=state.rounds.length,percent=Math.round(state.score/Math.max(total,1)*100),starCount=percent===100?3:percent>=70?2:percent>50?1:0,passed=starCount>0;updateStreak();const stats=read('stats',{best:0,played:0,xp:0,streak:0,lastDay:'',dailyDay:''});stats.best=Math.max(stats.best||0,percent);stats.played=(stats.played||0)+1;let bonus=0;if(state.special==='daily'){const today=LearningStore.day();if(stats.dailyDay!==today){bonus=25;stats.dailyDay=today}}stats.xp=(stats.xp||0)+state.xpEarned+bonus;write('stats',stats);
  ...
  function goHome(){if(screens.quiz.classList.contains('active'))saveQuiz();LearningAudio.stop();$('progress').style.width='0%';show('home');renderHome()}
  ...
  function saveQuiz(){if(state.rounds.length && state.index<state.rounds.length)LearningStore.saveSession('quiz',{profile:state.profile,level:state.level,mission:state.mission,rounds:state.rounds,index:state.index,score:state.score,hearts:state.hearts,xpEarned:state.xpEarned,validated:state.validated,quick:state.quick,special:state.special});}
  function resumeQuiz(){const saved=LearningStore.getSession('quiz');if(!saved||saved.profile!==state.profile||!LEVELS[saved.level]||!Array.isArray(saved.rounds)||!saved.rounds.length||saved.rounds.length>100)return;Object.assign(state,saved);state.index=Math.max(0,Number(state.index)||0);if(saved.validated)state.index++;if(state.index>=state.rounds.length){finishSession();return;}show('quiz');renderRound();}
  window.addEventListener('pagehide',()=>{if(screens.quiz.classList.contains('active'))saveQuiz();});
  state.level=localStorage.getItem(key('selectedLevel'))||'debutant';if(!LEVELS[state.level])state.level='debutant';window.appBack=goHome;renderHome();
  document.querySelectorAll('[data-profile]').forEach(b=>b.classList.toggle('selected',b.dataset.profile===state.profile));
  const resumeButton=$('resumeQuiz');resumeButton.hidden=!LearningStore.getSession('quiz');resumeButton.onclick=resumeQuiz;
  if(new URLSearchParams(location.search).has('resume'))resumeQuiz();
})();
